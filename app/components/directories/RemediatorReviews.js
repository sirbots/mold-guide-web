...
const getRemediationReviews = cache((remediationId) =>
  fetch(
    "/api/reviews/remediation/reviews-with-name/by-remediation-id/" +
      remediationId
  ).then((res) => res.json())
);

const RemediatorReviews = ({ remediationId }) => {
  let reviews = use(getRemediationReviews(remediationId));
  console.log(reviews);
...
export default RemediationReviews;
